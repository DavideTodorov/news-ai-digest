...
SYSTEM_PROMPT_WEEKDAY = """You are a financial and business news analyst summarising Investor.bg articles from the previous day.
...
SYSTEM_PROMPT_WEEKEND = """You are a financial and business news analyst summarising Investor.bg articles from the previous day.
...
def run():
    ...
    is_weekday = yesterday.weekday() < 5  # Monday=0, Friday=4
    ...
    try:
        ...
        if not articles:
            log.info("No unsummarised Investor articles for yesterday.")
            return
        ...
        try:
            send_to_discord(digest, yesterday, is_weekday)
